...
import {
  createOverlayElement, updateOverlayElement, updateOverlayLabel,
  repositionOverlay, removeOverlayElement, createPickerOverlay,
  injectAnimationSheet,
} from './overlay-renderer.js';
import { fieldColor, stepColor, COLOR_CAPTCHA, COLOR_BLOCKED, ZONE_PALETTE }
...
function _evictLRU() {
  ...
  for (const [k, z] of _zones) {
    if (z.insertedAt < oldestTime) { oldestTime = z.insertedAt; oldestKey = k; }
  }
  if (oldestKey) {
...
async function _loadPrefs() {
  try {
    const { fs_overlay_prefs: p } = await chrome.storage.local.get(['fs_overlay_prefs']);
...
export const overlayEngine = {
   ...
   */
  register({ selector, label, stepIndex = 0, fieldIndex = 0, mode = 'preview', color }) {
    ...
    const overlayEls = elements.map((el, i) => {
      ...
      const lbl = i === 0 && isMulti && _prefs.showMatchCount
        ? `${label} · ×${elements.length}` : label;
      return createOverlayElement(_shadowRoot, rects[i], resolvedColor, mode,
        showLabel ? lbl : '', isMulti, elements.length);
    });

    /** @type {ZoneDescriptor} */
    const zone = {
      zoneId, selector, color: resolvedColor, label, mode,
      elements, rects, overlayEls,
      stepIndex, fieldIndex, isMulti, matchCount: elements.length,
      insertedAt: Date.now(),
    };
    _zones.set(zoneId, zone);
    logger.debug(MODULE, 'zone-registered', { zoneId, selector, matchCount: elements.length });
    return zoneId;
  },
  ...
  setMode(zoneId, mode, errorMessage) {
    ...
    for (const div of zone.overlayEls) {
      updateOverlayElement(div, zone.color, mode, zone.label, errorMessage, zone.isMulti, zone.matchCount);
    }

    if (mode === 'completed' && _prefs.autoFadeCompleted) {
      setTimeout(() => this.unregister(zoneId), _prefs.fadeDelayMs ?? AUTO_FADE_COMPLETED_MS);
    }
  },
   ...
   */
  clearStep(stepIndex) { this.clearAll(stepIndex); },
  ...
  previewStep(zones, stepIndex) {
    ...
    for (const { selector, label, fieldIndex, color } of zones) {
      const zoneId = this.register({ selector, label, stepIndex, fieldIndex, mode: 'preview', color });
      const zone = _zones.get(zoneId);
  ...
  async previewAll(steps) {
    ...
    for (const [stepIdx, step] of steps.entries()) {
      ...
      for (const zone of zones) {
        const zoneId = this.register({ ...zone, stepIndex: stepIdx, mode: 'preview' });
  ...
  async reloadPrefs() { await _loadPrefs(); },

  /** Expose zone stats for dashboard. */
  getStats() {
    let live = 0, preview = 0, completed = 0, error = 0, blocked = 0;
  ...
  switch (type) {
    ...
    case 'overlay:setMode':
      if (payload.action === 'previewAll' && Array.isArray(payload.steps)) {
        const result = overlayEngine.previewAll ? overlayEngine.previewAll(payload.steps) : { matched: [], unmatched: [] };
    ...
    case 'scrapeZone:preview': {
      const result = overlayEngine.previewStep(payload.zones, payload.stepIndex);
      respond({ ok: true, ...result });
      break;
    }
